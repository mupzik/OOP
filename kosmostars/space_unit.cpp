...
star::star(std::string UnitName, float mmass, unsigned int pplanets, float vvelocity, float aage)
{
	...
	else
	{
		...
		temperature = pow((pow(radius, 5.2) / (4 * 3.14 * pow(radius, 2) * 5.67E-8)), 0.25);
		velocity = vvelocity;
		planets = pplanets;
		age = aage;
		name = UnitName;
...
string planet::earthquake() const
{
...
void planet::impact(space_unit* unit)
{
	if (unit->get_mass() > (0.3 * mass))
		 set_mass(0.2 * unit->get_mass() + mass);
	else
		set_mass(unit->get_mass() + mass);
}
...
void test_st()
{
	...
	assert(66547.172 >= test.get_temperature() >= 66547.170);
